...
|------|-------------------|----------------------|-----------------|
| **A01** | Broken Access Control | Middleware `proxy.ts` (redirige `/dashboard`, `/admin` vers `/auth/login`) ; Server Actions vérifient l'identité via la session Appwrite ; permissions par collection Appwrite ; contrainte d'unicité métier (`isUserInTeamForJam`) avant écriture. | `frontend/src/proxy.ts`, `src/lib/actions/*` |
