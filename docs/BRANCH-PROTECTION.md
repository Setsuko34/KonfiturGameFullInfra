...
> **Prérequis** : `gh auth login` effectué. Toutes les commandes ci-dessous se lancent depuis n'importe quel répertoire (le repo est passé explicitement).

---

## ⚠️ Limitation plan Free + repo privé
...
Options : passer le repo en public, ou prendre GitHub Pro. À vérifier après tout changement de plan/visibilité avec un test simple (tenter un push direct sur `main`).
...
| `Prod` | `14894070` | **AUCUNE** (`include` vide) | deletion, non_fast_forward | ⚠️ Inopérant — créé le 2026-04-10, jamais ciblé sur une branche. Supprimable : `gh api repos/Setsuko34/KonfiturGameFullInfra/rulesets/14894070 --method DELETE` |

Pour re-vérifier l'état réel à tout moment :
...
|------------|-----------|----------|
| PR obligatoire avant merge | `pull_request` | Aucun déploiement sans passer par une PR validée |
| Status checks requis (les 4 bloquants) | `required_status_checks` | `Lint + type-check`, `Tests unitaires`, `Scan secrets (gitleaks)`, `Checklist RGPD` |
| Branche à jour avant merge | `strict_required_status_checks_policy: true` | Ce qui est testé = ce qui est déployé |
| Force push interdit | `non_fast_forward` | Le `paths-filter` du job `changes` redéploie tout sur un force-push sans ancêtre commun, et le `git pull --ff-only` du VPS casserait |
...
- **Ne jamais ajouter** les jobs « non bloquant » (Semgrep, audit deps, lint Docker) ni les jobs `deploy-*` comme checks requis : les premiers sont informatifs, les seconds ne tournent que sur push main.
- **`integration_id: 15368`** = GitHub Actions. Garantit que seul Actions peut fournir ces checks (anti-usurpation).
- **`required_approving_review_count: 0`** : la PR est obligatoire mais pas l'approbation — indispensable en solo (impossible d'approuver sa propre PR).
...
```bash
# Lister tous les rulesets (id, nom, statut)
gh api repos/Setsuko34/KonfiturGameFullInfra/rulesets --jq '.[] | {id, name, enforcement}'

# Détail complet d'un ruleset
gh api repos/Setsuko34/KonfiturGameFullInfra/rulesets/<ID>

# Quelles règles s'appliquent effectivement à une branche donnée
gh api "repos/Setsuko34/KonfiturGameFullInfra/rules/branches/main"
```

Interface web : `https://github.com/Setsuko34/KonfiturGameFullInfra/settings/rules` (Settings → Rules → Rulesets).

---

## Modifier

`PUT` remplace la config envoyée. Le plus sûr : récupérer le JSON existant, l'éditer, le renvoyer.

```bash
# 1. Exporter la config actuelle
gh api repos/Setsuko34/KonfiturGameFullInfra/rulesets/<ID> > /tmp/ruleset.json

# 2. Éditer /tmp/ruleset.json (ne garder que : name, target, enforcement, conditions, rules, bypass_actors)

# 3. Renvoyer
gh api repos/Setsuko34/KonfiturGameFullInfra/rulesets/<ID> --method PUT --input /tmp/ruleset.json
```

Exemple — réparer le ruleset `Prod` existant pour qu'il cible enfin `main` :

```bash
gh api repos/Setsuko34/KonfiturGameFullInfra/rulesets/14894070 --method PUT --input - <<'EOF'
{
  "name": "Prod",
  "target": "branch",
  "enforcement": "active",
...
Utile pour un hotfix d'urgence — repasser `enforcement` à `"disabled"` puis `"active"` :
...
(`"evaluate"` existe aussi : mode audit, les violations sont journalisées mais pas bloquées — réservé aux plans payants.)
...
Aucune confirmation demandée — vérifier l'ID avec la commande de listing avant.
...
| Un job du workflow est renommé | Mettre à jour le `context` correspondant dans les deux rulesets (sinon PR bloquée en « Expected ») |
